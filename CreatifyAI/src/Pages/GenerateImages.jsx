...
function GenerateImage() {
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} action="" className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
       ...
       <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>{imageStyle.map((item)=>(
        ...
        className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-purple-50 text-green-500 pt-1.5' : 'text-gray-300 border-gray-300 pt-1.5'}`} key={item}>{item}</span>
       ))}
       </div>
  ...
  <label className="relative inline-flex items-center cursor-pointer">
    ...
    {/* Knob (needs to be inside the same label for peer to work) */}
      ...
      {!content ? (
       <div className='flex-1 flex justify-center items-center'>
        ...
        <div className='mt-3 h-full'>
          <img src={content} alt="Generated" className="mt-3 max-h-96 w-full object-contain"/>
        </div>
      )}
      </div>
    </div>
